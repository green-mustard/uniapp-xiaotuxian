...
onReady(() => {
  ...
  pageInstance.animate(
    '.navbar',
    [{ backgroundColor: 'tansparent' }, { backgroundColor: '#f8f8f8' }],
    1000,
    {
  ...
  // 动画效果，导航栏标题
  pageInstance.animate('.navbar .title', [{ color: 'tansparent' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 100,
...
const onPayOrder = async () => {
  if (isDEv) {
    ...
    // 正式环境的微信支付
    const res = await getWxPayMiniPayAPI({ orderId: query.id })
    // console.log(res)
...
<template>
  <!-- 自定义导航栏: 默认透明不可见, scroll-view 滚动到 100 时展示 -->
    ...
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="onPayOrder">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 -->
        <template v-else>
          <!-- 订单状态文字 -->
